...
public class ShortestDistanceToTargetStringInACircularArray_2515 {
    public static void main(String[] args) {
        Solution solution = new ShortestDistanceToTargetStringInACircularArray_2515().new Solution();
    }
    ...
    class Solution {
        public int closestTarget(String[] words, String target, int startIndex) {
            ...
            List<Integer> list = m.get(target);
            int ans = Integer.MAX_VALUE;
            for (int i = 0; i < list.size(); i++) {
                ans = Math.min(ans,Math.abs(list.get(i) - startIndex));
                ans = Math.min(ans,Math.abs(words.length - Math.abs(list.get(i) - startIndex)));
            }
            return ans;
        }
    }
