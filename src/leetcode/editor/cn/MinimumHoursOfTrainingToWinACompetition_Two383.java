...
public class MinimumHoursOfTrainingToWinACompetition_Two383 {
    public static void main(String[] args) {
        Solution solution = new MinimumHoursOfTrainingToWinACompetition_Two383().new Solution();
    ...
    class Solution {
        public int minNumberOfHours(int initialEnergy, int initialExperience, int[] energy, int[] experience) {
            ...
            int energyH = Math.max(sum - initialEnergy + 1, 0);

            int exH = 0;
            int totalEx = initialExperience;
            for (int i = 0; i < n; i++) {
                int diff = experience[i] - totalEx + 1;
                totalEx += Math.max(0, diff);
                exH += Math.max(0, diff);
                totalEx += experience[i];
            }
