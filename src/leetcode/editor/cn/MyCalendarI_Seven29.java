...
public class MyCalendarI_Seven29 {
    ...
    class MyCalendar {
        ...
        TreeSet<int[]> bookeds = new TreeSet<>(Comparator.comparingInt(val -> val[0]));
        ...
        public boolean book(int start, int end) {
            if (bookeds.isEmpty()) {
                bookeds.add(new int[]{start, end});
                return true;
            }

            int[] tmp = {end, 0};
            int[] next = bookeds.ceiling(tmp);
            int[] prev = bookeds.lower(tmp);

            if (next == bookeds.first() || (prev != null && prev[1] <= start)) {
                bookeds.add(new int[]{start, end});
                return true;
            }
            return false;
