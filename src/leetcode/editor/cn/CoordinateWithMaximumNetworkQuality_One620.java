...
public class CoordinateWithMaximumNetworkQuality_One620 {
    public static void main(String[] args) {
        Solution solution = new CoordinateWithMaximumNetworkQuality_One620().new Solution();
    }
        ...
        public int[] bestCoordinate(int[][] towers, int radius) {
            ...
            for (int i = 0; i <= 50; i++) {
                for (int j = 0; j <= 50; j++) {
                    int s = 0;
                    for (int[] t : towers) {
                        double d = Math.sqrt((i - t[0]) * (i - t[0]) + (j - t[1]) * (j - t[1]));
                        if (d <= radius) {
                            s += Math.floor(t[2] / (1 + d));
                        }
                    }
                    if (sum < s) {
                        sum = s;
