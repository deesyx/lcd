...
public class ExpressiveWords_Eight09 {
    public static void main(String[] args) {
        Solution solution = new ExpressiveWords_Eight09().new Solution();
    ...
    class Solution {
        ...
        private boolean check(String s, String word) {
            ...
            while (i < s.length() && j < word.length()) {
                ...
                int counti = 0;
                int countj = 0;
                while (i < s.length() && s.charAt(i) == curi) {
                    counti++;
                    i++;
                }
                while (j < word.length() && word.charAt(j) == curj) {
                    countj++;
                    j++;
                }
