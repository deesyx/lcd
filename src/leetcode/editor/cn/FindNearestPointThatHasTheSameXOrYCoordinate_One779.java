...
public class FindNearestPointThatHasTheSameXOrYCoordinate_One779 {
    public static void main(String[] args) {
        Solution solution = new FindNearestPointThatHasTheSameXOrYCoordinate_One779().new Solution();
    }
    ...
    class Solution {
        public int nearestValidPoint(int x, int y, int[][] points) {
            ...
            for (int i = 0; i < points.length; i++) {
                ...
                int val = Math.abs(x - a) + Math.abs(y - b);
                if (val < min) {
                    min = val;
                    ans = i;
                }
